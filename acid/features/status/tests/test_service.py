...
import acid.status.service
import acid.status.tests.fixtures
from acid.config import config
from acid.status.exceptions import (PipelineNotFound,
                                    RemoteServerError)
from acid.status.model import PipelineStat


class TestServicePipelineStats(TestCase):
    def test_pipelines_stats_returns_empty_when_no_pipelines(self):
        resource = acid.status.tests.fixtures.load_status_data(
            name='status_no_pipelines')
        expected = []
        result = acid.status.service.pipelines_stats(
...
class TestServiceEndpointStatus(TestCase):
    def test_status_endpoint_returns_expected_when_no_slashes(self):
        with patch.dict(config['zuul'], {'url': 'http://fake.url',
            ...
            result = acid.status.service.status_endpoint()
...
@patch('acid.status.service.current_app')
@patch('acid.status.service.requests')
class TestServiceFetchData(TestCase):
    def test_fetch_raise_when_cant_download(self, requests, *args):
        ...
        with self.assertRaises(RemoteServerError):
            acid.status.service.fetch_json_data(
                endpoint='http://fake.endpoint')

    def test_fetch_return_expected_data(self, requests, *args):
        requests.get = acid.status.tests.fixtures.status_request(
            filename='status_check')

        result = acid.status.service.fetch_json_data(
            endpoint='http://fake.endpoint')
        expected = acid.status.tests.fixtures.load_status_data(
            name='status_check')
...
class TestServiceMakeQueues(TestCase):
    def test_raises_when_no_queues(self, *args):
        resources = acid.status.tests.fixtures.load_status_data(
            name='status_no_queues')
        with self.assertRaises(KeyError):
            acid.status.service.make_queues(
